...
SELECT * FROM Financial.DailyPortfolioAnalysis
GO
...
SELECT YEAR(T.[TransDate]) AS TransYear
  ...
  JOIN [MasterData].[AccountType] PAT
  ON T.PortfolioAccountTypeCode = PAT.AcctTypeCode
...
-- 54057 rows in [MasterData].[TickerPriceRangeHistoryDetail]
...
TRUNCATE TABLE Financial.NtileSymbolBuckets
GO

INSERT INTO Financial.NtileSymbolBuckets
SELECT YEAR([TransDate]) AS TransYear
	  ,[Symbol]
      ,SUM([TransAmount]) AS SumTransactions
	  ,NTILE(9) OVER (
...
FROM Financial.NtileSymbolBuckets
GO
...
;WITH SymbolBuckets(TransYear,Symbol,InvestmentBucket,SumTransactions)
AS
(
SELECT N.TransYear	AS TransYear,
	N.Symbol		AS Symbol,
	N.InvestmentBucket,
	SUM(N.SumTransactions) AS SumTransactions
FROM Financial.NtileSymbolBuckets N
CROSS JOIN [MasterData].[Calendar]
CROSS JOIN [Financial].[TickerSymbols]
GROUP BY N.TransYear,
	N.Symbol,
	N.InvestmentBucket
)
...
SELECT TransYear,
	...
	RANK() OVER(
			PARTITION BY TransYear,InvestmentBucket
			ORDER BY InvestmentBucket,SumTransactions ASC
		) AS InvestmentRank
FROM SymbolBuckets
GO
