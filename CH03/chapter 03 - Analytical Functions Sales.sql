...
DROP TABLE IF EXISTS [dbo].[CumDist] 
GO
...
DROP INDEX IF EXISTS [CDCustYearQtrMonTotal>]
ON [dbo].[CumDist]
GO

CREATE NONCLUSTERED INDEX [CDCustYearQtrMonTotal>]
ON [dbo].[CumDist] ([CustomerNo],[SalesYear])
...
DROP INDEX IF EXISTS ieSalesDateCustStore ON StagingTable.SalesTransaction
GO

DROP INDEX IF EXISTS [CDCustYearQtrMonTotal>] ON [dbo].[CumDist]
GO

DROP TABLE IF EXISTS CumDist
GO
...
DROP INDEX IF EXISTS ieCustomerStoreDateAmount
...
-- execute command only if file group does not exists 

/*
ALTER DATABASE APSales
ADD FILEGROUP AP_SALES_MEMOPTpt CONTAINS MEMORY_OPTIMIZED_DATA;
GO
*/
...
-- execute command only if file does not exists 

/*
ALTER DATABASE APSales 
ADD FILE (
name='APSalesMemoOptData', 
filename=N'D:\APRESS_TOOLKIT_DATABASES\AP_SALES\MEMORYOPT\APSalesMemOpt.mdf'
)
TO FILEGROUP AP_SALES_MEMOPTpt
GO
*/
...
SELECT * FROM [SalesReports].[MemorySalesTotals]
GO
...
TRUNCATE TABLE SalesReports.YearlySummaryReport
GO

INSERT INTO SalesReports.YearlySummaryReport
SELECT YEAR(CalendarDate) AS SalesYear
	  ,MONTH(CalendarDate) AS SalesMonth
      ,StoreNo
      ,StoreName
...
DROP INDEX IF EXISTS [ieYearlySalesStoreTerritorySummary]
ON [SalesReports].[YearlySummaryReport]
GO
...
FROM SalesReports.YearlySummaryReport
WHERE SalesYear IN(2010,2011)
AND StoreNo = 'S00004'
GO
