...
-- this will load the original table without the text columns
-- these are added later on in the script
...
-- if the text columns have been added use this
...
SELECT * FROM APInventoryWarehouse.Dimension.Calendar
GO

-- now update the APInventory Database Calendar
...
-- use below in scripts that have TRY/CATCH logic so you can trap errors
...
CREATE TABLE #ErrorExample (ColValue INT);
GO

BEGIN TRY
	INSERT INTO #ErrorExample VALUES('One');
END TRY
...
DROP INDEX IF EXISTS ieWhIdProdIdMovementDate
ON Product.InventoryTransaction
GO

CREATE NONCLUSTERED INDEX ieWhIdProdIdMovementDate
ON Product.InventoryTransaction (WhId,ProdId)
...
-- this uses the data warehouse APInventoryWarehouse
